...
local function drawTitle()
	if menus[currentMenu] then
		...
		if HasStreamedTextureDictLoaded("menu_textures") then
			DrawSprite("generic_textures", "menu_header_1a", x, y, menus[currentMenu].width + .03, titleHeight, 0.0, 150, 2, 2, 255, 0)
		end
...
local function drawSubTitle()
	if menus[currentMenu] then
		...
		if HasStreamedTextureDictLoaded("generic_textures") then
			...
			DrawSprite("generic_textures", "selection_box_bg_1c", x, y - 0.002, menus[currentMenu].width + 0.01 , buttonHeight, 0.0, 0, 0, 0, 255, 0)
		end
...
local function drawDescription(text)
	...
	local _, lines = drawMultilineFormat(text)
...
local function drawArrows()
    ...
    if HasStreamedTextureDictLoaded("generic_textures") then
		...
		if HasStreamedTextureDictLoaded("menu_textures") then
			SetScriptGfxDrawOrder(0)
			DrawSprite("menu_textures", "translate_bg_1a", x, y, menus[currentMenu].width + 0.02, buttonHeight, 0.0, 255, 0, 0, 255, 0)
		end

		DrawSprite("menu_textures", "diamond", x, y, 0.0200, 0.04, 0.0, 255, 255, 255, 255, 0)
...
function WarMenu.Button(text, subText, descText)
	...
	if menus[currentMenu] then
		...
		debugPrint('Impossible de créer '..tostring(text)..' Bouton: '..tostring(currentMenu)..' car le Menu n\'existe pas')
...
function WarMenu.MenuButton(text, id)
	if menus[id] then
		...
		debugPrint('Impossible de créer '..tostring(text)..' Bouton: '..tostring(id)..' car le Menu n\'existe pas')
	end
...
function WarMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if WarMenu.Button(text, tostring(selectedItem)) then
