-- IT'S JUST A SCRIPT TEST FOR ME, NOT USED BY THE TATTOOS SCRIPT
...
local overlays_info = {
    ...
    ["eyebrows"] = {
        ...
        {id=0x8A4B79C2,albedo=0xAE6ED4E6,nonrmal=0x89B29E5A,ma=0xFA0476E4,},    --   {albedo="mp_u_faov_eyebrow_f_002_ab",normal="mp_u_faov_eyebrow_f_002_nm",ma="mp_u_faov_eyebrow_f_002_ma",},
...
-- DONT FORGET TO CHANGE OVERLAYS TO DEFAULT TABLE, IF U CHANGE PED.
...
-- arg[8] palette_color_secondary:     from 0 till 254
-- arg[9] palette_color_tertiary:      from 0 till 254
...
if Config.DEBUG then
    RegisterCommand('ov', function(source, args)
        if args [1] ~= nil then
            ...
            local visibility = 1
            if args[2] ~= nil then
                visibility = tonumber(args[2])
            end
            local tx_id = 1
            if args[3] ~= nil then
                tx_id = tonumber(args[3])
            end
            local var = 0
            if args[4] ~= nil then
                var = tonumber(args[4])
            ...
            if args[7] ~= nil then
                palette_color_primary = tonumber(args[7])
            end
